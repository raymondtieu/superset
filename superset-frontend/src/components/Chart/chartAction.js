...
export async function getChartDataRequest({
  ...
  // Dev-only: allow mocking DEX chart data from the frontend without hitting the backend.
  // Toggle by setting localStorage.DEX_MOCK = '1' in the browser console.
  try {
    const isDex = formData?.viz_type === 'dex';
    const shouldMockDex =
      isDex &&
      typeof window !== 'undefined' &&
      window?.localStorage?.getItem('DEX_MOCK') === '1';
    if (shouldMockDex) {
      // Build minimal mock that satisfies both chart rendering and Results pane expectations
      const now = new Date();
      ...
      const baseByPlatform = {
        '<NULL>': 30_000_000,
        ANDROID_MOBILE: 20_000_000,
        ANDROID_TABLET: 2_000_000,
        IPAD: 3_000_000,
        IPHONE: 22_000_000,
        WEB_DENZEL: 11_000_000,
        WEB_MOBILE: 9_000_000,
      };
      ...
      const json = {
        result: [
          // queriesData[0] → EchartsTimeseries (can consume long-form rows too)
          {
            is_cached: false,
            cache_key: null,
            cached_dttm: null,
            ...
            status: 'success',
            data: mockRows,
            colnames: ['dt', 'platform', 'NUM_PIN_DELETE'],
          },
          // queriesData[1] → PivotTable in DEX
          {
            is_cached: false,
            cache_key: null,
            cached_dttm: null,
    ...
    // If anything goes wrong in the mocking block, fall through to normal request
    // to avoid breaking the normal flow.
  }
